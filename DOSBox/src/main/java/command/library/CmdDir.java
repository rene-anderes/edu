...
import command.framework.Wrapper;
import filesystem.Directory;
import filesystem.Drive;
import filesystem.FileSystemItem;

/**
 * Inhalt von Laufwerken und Verzeichnissen mit verschiedenen Optionen
 * auflisten.
 * 
 * @author ra, hum
 */
class CmdDir extends Command {
    ...
    private void printDirectory(Outputter outputter, ArrayList<FileSystemItem> content, Directory dir) {
        ...
        if (options.contains(Option.COMPACT)) {
            Outputter out = new Wrapper(outputter, 60);
            printCompactItem(out, it);
        } else {
